...
do_covariance <- function(gene_id,chrom,chrom_list,gt_cov_cis_df, cov_geno_dir, rsids, varIDs, out_file) {
	cis_varID_list=c()
        index=1
        for (k in 1:length(varIDs)){
             tmp_chrpos<-varIDs[k]
             tmp_chrom<-unlist(strsplit(tmp_chrpos, "_"))[1]
             if (tmp_chrom == chrom){
                  cis_varID_list[index]=tmp_chrpos
                  index=index+1
             }
        }
	model_gt <- gt_cov_cis_df[,cis_varID_list, drop=FALSE]
	if (length(chrom_list)!=0){
  		for (i in (1:length(chrom_list))){
			curr_chrom<-chrom_list[i]
	 		cov_geno_file<-cov_geno_dir%&%"GTEx_v8_866Indiv_maf0.01_tped_chr" %&% curr_chrom %&% ".num.csv"
	  		gt_cov_df<-get_covariance_genotype(cov_geno_file)
			tmp_varID_list=c()
			index=1
                        for (k in 1:length(varIDs)){
                             tmp_chrpos<-varIDs[k]
                             tmp_chrom<-unlist(strsplit(tmp_chrpos, "_"))[1]
			     if (tmp_chrom == curr_chrom){
				     tmp_varID_list[index]=tmp_chrpos
				     index=index+1
			     }
			}
  	  		tmp_model_gt <- gt_cov_df[,tmp_varID_list, drop=FALSE]
 	  		model_gt<-cbind(model_gt,tmp_model_gt)
  		}
	}
...
sel_trans_group <- function(target_gene_id,data_trans_dir,adj_expression) {
	data_TF_dir<-data_trans_dir%&%"TF_trans/Gene_ID/"%&%target_gene_id%&%"/"
        inCsvfile_TF <-data_TF_dir%&%target_gene_id%&%".sel_trans.csv"

	sel_full_list=c()
	df_bool=FALSE
	if (file.exists(inCsvfile_TF)==TRUE) {
    		groups_all=c()
    		if (file.exists(inCsvfile_TF)==TRUE){
      			df_geno_TF <- read.table(file=inCsvfile_TF,header=TRUE,sep=",",check.names = FALSE)
      			tmp_tf_group<-strsplit(as.character(df_geno_TF$TF),'_')
                tmp_1_list=vapply(tmp_tf_group, paste, collapse = ", ", character(1L))
                groups_tf_list=c()
                for (k in 1:length(tmp_1_list)){
                    tmp_1=tmp_1_list[k]
                    tmp_tf_1<-unlist(strsplit(tmp_1, ","))[1]
                    groups_tf_list[k]=tmp_tf_1
                }
      			df_geno_new <-df_geno_TF
      			df_bool=TRUE
      			groups_all <-c(groups_tf_list)
  		 }
   		list_trans=c()
		groups_all <- as.factor(groups_all)
  		df_geno_T<- as.data.frame(t(as.matrix(df_geno_new[, c(-1,-2,-3)])))
  		colnames(df_geno_T) <- df_geno_new$TF

  		cvfit <- tryCatch(cv.grpreg(as.matrix(df_geno_T), adj_expression, group=groups_all, nfolds = 10),
                    error = function(cond) {message('Error'); message(geterrmessage()); list()})
  		nzero_group <- predict(cvfit, type="ngroups", lambda=cvfit$lambda.min)  # Number of nonzero groups
  		nzero_group_factor <-predict(cvfit, type="groups", lambda=cvfit$lambda.min)  # Number of nonzero groups
  		nzero_group_list <- vapply(nzero_group_factor, paste, collapse = ", ", character(1L))
		
		sel_tf_list=c()
  		if (file.exists(inCsvfile_TF) && nzero_group >0){
    			df_geno_TF <- read.table(file=inCsvfile_TF,header=TRUE,sep=",",check.names = FALSE)
    			ori_col<-vapply(df_geno_TF$TF, paste, collapse = ", ", character(1L))
    			sel_tf_list=c()
    			index=1
    			for(i in 1:length(nzero_group_list)) {
      				curr_tf = nzero_group_list[i]
      				for (j in 1:length(ori_col)){
        				ori_tf = ori_col[j]
        				if(grepl(curr_tf, ori_tf, fixed = TRUE)){
          					sel_tf_list[index]=ori_tf
          				index=index+1
        				}
      				}	
    			}
  		}
  		sel_trans_all_list=c(sel_tf_list)
		sel_trans_all_list
	}
...
get_trans_genotype <- function(target_gene_id,cis_gt, data_trans_dir,sel_full_list) { 
	cis_snp_name<-colnames(cis_gt)
	cis_gt_T<- as.data.frame(t(as.matrix(cis_gt[,])))
	list_cis=c()
	for (i in 1:nrow(cis_gt_T)){
  		list_cis[i]=paste("cis",i-1,sep="_")
	}
	df_geno_cis<-cbind(SNP=cis_snp_name,TF = list_cis, cis_gt_T)
	sel_final_list<-c(list_cis,sel_full_list)
	df_geno_new=df_geno_cis

	data_TF_dir<-data_trans_dir%&%"TF_trans/Gene_ID/"%&%target_gene_id%&%"/"
        inCsvfile_TF <-data_TF_dir%&%target_gene_id%&%".sel_trans.csv"
	sel_full_list=c()
	if (file.exists(inCsvfile_TF)==TRUE){
  		df_geno_TF0 <- read.table(file=inCsvfile_TF,header=TRUE,sep=",",check.names = FALSE)
		snp_name<-df_geno_TF0$CHR %&% "_"%&%df_geno_TF0$LOC
		df_geno_TF <-df_geno_TF0[, c(-2,-3)]
		df_geno_TF_new=cbind(SNP=snp_name,df_geno_TF)
  		df_geno_new <-rbind(df_geno_new,df_geno_TF_new)
	}
	rownames(df_geno_new)=df_geno_new$TF
	#print(head(df_geno_new))
	df_geno_new_select <- df_geno_new[sel_final_list,]
	sel_snp_names<-df_geno_new_select$SNP
	df_geno_T<- as.data.frame(t(as.matrix(df_geno_new_select[, c(-1,-2)])))
	colnames(df_geno_T) <- sel_snp_names
	df_geno_T
}
### main function #####
...
gt_cov_cis_df<-get_covariance_genotype(covariance_genotype_file)
...
trans_file <- './summary/' %&% prefix %&% '_chr' %&% chrom %&% '_trans.txt'
trans_col <- c('GENE', 'TRANS')
write(trans_col, file = trans_file, ncol = 2, sep = '\t')
...
for (i in 1:n_genes) {
  ...
  if(length(dim(cis_gt))==2){
  	if (ncol(cis_gt) >= 2) {
		...
		sel_trans_all_list<-sel_trans_group(gene,data_trans_dir,adj_expression)
		if(length(sel_trans_all_list)!=0){
			list_geneid=c()
			list_trans_id=c()
                	for (j in 1:length(sel_trans_all_list)){
                		list_geneid[j]=gene
				list_trans_id[j]=sel_trans_all_list[j]
                	}	
			trans_df<-cbind(list_geneid,list_trans_id)
			write.table(trans_df, file = trans_file, append = TRUE, quote = FALSE, col.names = FALSE, row.names = FALSE, sep = "\t")
		}
		trans_gt<-get_trans_genotype(gene,cis_gt, data_trans_dir,sel_trans_all_list) 
		#adj_expression <- adjust_for_covariates(expression_vec, covariates_df)
    		elnet_out <- do_elastic_net(trans_gt, adj_expression, n_k_folds, cv_fold_ids, n_times, alpha)

    		if (length(elnet_out) > 0) {
      			eval <- evaluate_performance(trans_gt, adj_expression, elnet_out$cv_fit, elnet_out$best_lam_ind, elnet_out$best_lambda, cv_fold_ids, n_k_folds)
			model_summary <- c(gene, as.character(gene_name), alpha, elnet_out$min_avg_cvm, elnet_out$best_lam_ind,
      			...
      			if (eval$n_weights > 0) {
        			weighted_snps_info <- full_snp_annot %>% filter(chrpos %in% eval$weighted_snps) %>% select(SNP,chrpos,ref,effect)
				if (nrow(weighted_snps_info) != 0){
        				weighted_snps_info$gene <- gene
        				weighted_snps_info <- weighted_snps_info %>% merge(data.frame(weights = eval$weights, chrpos=eval$weighted_snps), by = 'chrpos') %>%
        				...
        				chrom_list=c()
					index=1
					for (k in 1:length(weighted_snps_info$chrpos)){
                               tmp_chrpos<-weighted_snps_info$chrpos[k]
                               tmp_chrom<-unlist(strsplit(tmp_chrpos, "_"))[1]
						if (tmp_chrom!=chrom){
							if (!tmp_chrom %in% chrom_list) {
								chrom_list[index]=tmp_chrom
								index=index+1
							}
						}
                        		}
					do_covariance(gene,chrom, chrom_list,gt_cov_cis_df, covariance_genotype_dir, weighted_snps_info$SNP, weighted_snps_info$chrpos, covariance_file)
